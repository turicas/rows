...
from rows.utils.date import (
    date_range, next_year, last_year, next_month,
    last_month, next_week, last_day, next_day, last_week)
...
def test_date_range():
    ...
    expected = [datetime.date(2020, 1, 1), datetime.date(2020, 2, 1), datetime.date(2020, 3, 1)]
    assert result == expected
    ...
    result = list(date_range(datetime.date(2020, 1, 1), datetime.date(2019, 12, 30), step=datetime.timedelta(days=-1)))
