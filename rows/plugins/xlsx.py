...
def _get_end_cell(max_cell, end_cell):
    if not end_cell:
        return max_cell

    if end_cell < 0:
        return max_cell + end_cell
    return end_cell
...
def import_from_xlsx(filename_or_fobj, sheet_name=None, sheet_index=0,
    ...
    end_row = _get_end_cell(sheet.max_row, end_row)
    start_col = max(start_column, sheet.min_column)
    end_col = _get_end_cell(sheet.max_column, end_column)
